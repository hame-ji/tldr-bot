...
class DigestGeneratorTests(unittest.TestCase):
    ...
    def test_ignored_count_token_renders(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            prompt_path = Path(tmpdir) / "digest.txt"
            prompt_path.write_text("IGNORED={{ignored_count}}\n{{summaries}}\n", encoding="utf-8")
            ...
            result = generate_digest(
                items=[
                    ...
                    {"status": "ignored", "url": "https://youtu.be/abc", "kind": "youtube"},
                ],
                run_date=date(2026, 3, 15),
                prompt_path=str(prompt_path),
                digests_base_dir=tmpdir,
            ...
            self.assertIn("IGNORED=1", result["digest_text"])
